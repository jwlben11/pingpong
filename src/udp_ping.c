...
recv_bytes=recv(ping_socket,answer_buffer,sizeof(answer_buffer),MSG_DONTWAIT);
	if (recv_bytes < 0 && errno != EAGAIN && errno != EWOULDBLOCK)
		fail_errno("UDP ping could not receive message");
...
/*** TO BE DONE START ***/
connect(ping_socket, (struct sockaddr*)pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen);
...
if (strcmp(answer, "OK") != 0)
    fail("Pong server did not agree to ping-pong");
